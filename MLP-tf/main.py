...
def load_dataset():
    train_data_reader = DataReader(
        data_path='../TF-IDF/20news-bydate/data_tf_idf_train.txt',
        batch_size=50,
        vocab_size=vocab_size
    )

    test_data_reader = DataReader(
        data_path='../TF-IDF/20news-bydate/data_tf_idf_test.txt',
        batch_size=50,
        vocab_size=vocab_size
    )
...
def save_parameters(name, value, epoch):
    ...
    with open('saved-params/' + filename, 'w') as f:
        f.write(string_form)


def restore_parameters(name, epoch):
    filename = name.replace(':', '-colon-') + '-epoch-{}.txt'.format(epoch)
    with open('saved-params/' + filename, 'r') as f:
